...
ORDERS = [
    ...
    # {'cred': 2 * 1000000, 'buildC': 3, 'shipC':18, 'model': 'TD-MK-I'}, # Fuel
    {
        "cred": 200 * 1000000,
        "buildC": 0,
        "build": "RESEARCH_OUTPOST",
        "location": "ZY1-GH",
    },
    {
        "cred": 200 * 1000000,
        "buildC": 1,
        "build": "ELECTRONICS_FACTORY",
        "location": "ZY1-GH-HD",
    },
    {
        "cred": 200 * 1000000,
        "buildC": 2,
        "build": "FABRICATION_PLANT",
        "location": "ZY1-GH-NT",
    },
    {
        "cred": 200 * 1000000,
        "buildC": 3,
        "build": "CHEMICAL_PLANT",
        "location": "ZY1-GH-MD",
    },
    {
        "cred": 200 * 1000000,
    ...
    {"cred": 200 * 1000000, "buildC": 5, "build": "MINE", "location": "ZY1-GG-CY"},
    {
        "cred": 200 * 1000000,
        "buildC": 6,
        "build": "RARE_EARTH_MINE",
        "location": "ZY1-T85",
    },
    {
        "cred": 200 * 1000000,
        "buildC": 7,
        "build": "EXPLOSIVES_FACILITY",
        "location": "ZY1-GG-LAO",
    },
    {
        "cred": 200 * 1000000,
...
def updateGalaxy(galaxy="OE"):
    ...
    if "shipListings" in js:
        for ship in js["shipListings"]:
            ...
            st.ships[ship["type"]]["purchaseLocations"].update(
                {i["location"]: i["price"] for i in ship["purchaseLocations"]}
            )
...
def buyItem(ship, good, quantity, location):
    ...
    while quantity > loadSpeed:
        js = callApi(
            "POST",
            "/my/purchase-orders",
            {"shipId": shipId, "good": good, "quantity": loadSpeed},
        )
        quantity -= loadSpeed
    js = callApi(
        "POST",
        "/my/purchase-orders",
        {"shipId": shipId, "good": good, "quantity": quantity},
    )
    st.stats["buy_" + good + "_" + location] = (
        st.stats.get("buy_" + good + "_" + location, 0) + quantity
    )
...
def sellItem(ship, good, quantity, location):
    ...
    st.stats["sell_" + good + "_" + location] = (
        st.stats.get("sell_" + good + "_" + location, 0) + quantity
    )
    if js.get("order", {}).get("total") and shipId in st.orders["_live"]:
        print(
            "trade summary %s %d for %d"
            % (
                good,
                quantity,
                js["order"]["total"] - st.orders["_live"][shipId]["price"],
            )
        )
...
def transferItem(fromShipId, toShipId, good, quantity):
    ...
    return callApi(
        "POST",
        "/my/ships/%s/transfer" % fromShipId,
        {"shipId": toShipId, "good": good, "quantity": quantity},
    )
...
def buildStructure(structureType, location):
    ...
    return callApi(
        "POST", "/my/structures", {"location": location, "type": structureType}
    )
...
def shipToStructure(ship, structureId, good, quantity, location):
    ...
    while quantity > loadSpeed:
        js = callApi(
            "POST",
            "/my/structures/%s/deposit" % structureId,
            {"shipId": shipId, "good": good, "quantity": loadSpeed},
        )
        quantity -= loadSpeed
    js = callApi(
    ...
    st.stats["deposit_" + good + "_" + location] = (
        st.stats.get("deposit_" + good + "_" + location, 0) + quantity
    )
...
def structureToShip(ship, structureId, good, quantity, location):
    ...
    while quantity > loadSpeed:
        js = callApi(
            "POST",
            "/my/structures/%s/transfer" % structureId,
            {"shipId": shipId, "good": good, "quantity": loadSpeed},
        )
        quantity -= loadSpeed
    js = callApi(
    ...
    st.stats["transfer_" + good + "_" + location] = (
        st.stats.get("transfer_" + good + "_" + location, 0) + quantity
    )
    return js
...
def setPath(shipId, destination, mkI):
    ...
    if "error" not in js:
        st.distances[
            js["flightPlan"]["departure"]
            + "|"
            + js["flightPlan"]["destination"]
            + "|"
            + str(mkI)
        ] = (
            js["flightPlan"]["fuelConsumed"],
            js["flightPlan"]["timeRemainingInSeconds"],
        )
...
def fuelConsumptionIntra(source, destination, shipType="MK-II"):
    ...
    return (
        math.ceil(math.sqrt(xDiff * xDiff + yDiff * yDiff) / 7.5)
        + len(shipType.rsplit("-", 1)[-1])
        + (2 if st.locations[source]["type"] == "PLANET" else 0)
    )
...
def getWormholePath(planetSrc, planetDest=None):
    ...
    return [
        random.choice(
            [
                i
                for i in st.locations.keys()
                if i.startswith(planetSrc.split("-")[0] + "-W-")
            ]
        )
    ]
...
def tradeWeight(trade, ship):
    if ship["maxCargo"] < trade["fuel"] * 2:
        return 0
    netGain = (ship["maxCargo"] - trade["fuel"]) * trade["gain"] - trade[
        "fuel"
    ] * fuelPrice(trade["src"])
    netGain -= getTotalFuelConsumption(
        ship["location"], trade["src"], ship["type"]
    ) * fuelPrice(ship["location"])
    if netGain < 0:
        return 0
        ...
        print(
            ...
            % (
                ...
                " ".join(
                    [
                        "%s(%s)" % (i[0], i[1])
                        for i in sJs.get("purchaseLocations", {}).items()
                    ]
                ),
                " ".join(sJs.get("restrictedGoods", [])),
            )
        )
    ...
    if js.get("shipCount", 0) > 0:
        ...
        if "ships" in sjs:
            ...
            for ship in sjs["ships"]:
                ...
                print(
                    "(%8s) %9s[%3s]%29s\t%s f:%d %s"
                    % (
                        ship.get("location", ""),
                        ship["type"],
                        st.orders["_rank"].get(ship["id"], "")[:3],
                        ship["id"],
                        currentTrade,
                        fuel,
                        cargoStr,
...
def getMarketplace(location):
    ...
    if "marketplace" in js:
        ...
        for item in js["marketplace"]:
            ...
            st.goods[item["symbol"]]["min"] = min(
                st.goods[item["symbol"]].get("min", item["pricePerUnit"]),
                item["pricePerUnit"],
            )
            st.goods[item["symbol"]]["max"] = max(
                st.goods[item["symbol"]].get("max", item["pricePerUnit"]),
                item["pricePerUnit"],
            )
            st.goods[item["symbol"]]["minSpread"] = min(
                st.goods[item["symbol"]].get("minSpread", item["spread"]),
                item["spread"],
            )
            st.goods[item["symbol"]]["maxSpread"] = max(
                st.goods[item["symbol"]].get("maxSpread", item["spread"]),
                item["spread"],
            )
                ...
                for cargo in ship["cargo"]:
                    if cargo["good"] != "FUEL":
                        if ship["id"] in st.orders["_live"]:
                            ...
                            sellItem(
                                ship, cargo["good"], cargo["quantity"], ship["location"]
                            )
                            ship["spaceAvailable"] += cargo["totalVolume"]
...
def decideExpansion():
    ...
    for order in ORDERS:
        if (
            order.get("minShipC", 0) <= shipCount <= order.get("shipC", 48)
            and buildCount == order.get("buildC", buildCount)
            and st.my["credits"] > order.get("cred", 0)
        ):
            if "model" in order:
                # buy ship
                if "location" in order:
                    ...
                    if "ship" in js:
                        st.my["credits"] = js["credits"]
                        st.orders["_rank"][js["ship"]["id"]] = order.get(
                            "role", "inter"
                        )
                        if "initial" in order:
                    ...
                    for store in set(
                        st.ships[order["model"]]["purchaseLocations"].keys()
                    ).intersection(st.my.get("_active_locations", [])):
                ...
                "loan" in order
                and st.loans.get("active")
                and st.loans["active"][0].get("id")
            ):
...
def findTrades():
            ...
            if inventory[good] < 50:  # TODO 100K produce varsa alma ?
                ...
                for fromLocId in marketIds:
                    trades.append(
                        {
                            "type": "intra"
                            if isSameSystem(fromLocId, toLocId)
                            else "inter",
                            ...
                            "gain": (
                                1.0
                                * gJs[fromLocId]["purchasePricePerUnit"]
                                / gJs["volumePerUnit"]
                            ),
                        }
                    )
    ...
    return sorted(
        trades, key=lambda x: x["gain"], reverse=True
    )  # [(gain/volume, s, d, item), ... ]
    ...
    for ship in st.my.get("ships", []):
        if "location" in ship and st.orders.get(ship["id"], []) == []:
            if st.orders["_rank"][ship["id"]] == "scout":
                ...
                expiredMarket = [
                    m
                    for m, mJs in st.market.items()
                    if currentTime - mJs.get("_update") > 1200 and "-W-" not in m
                ]
                dest = ""
                if unexplored:
                    dest = random.choices(
                        ...
                        list(
                            map(
                                lambda d: 100
                                - getTotalFuelConsumption(ship["location"], d),
                                unexplored,
                            )
                        ),
                    )
                    ...
                    if len(st.loans.get("active", [""])) == 0:
                        unexploredGalaxy = [
                            g for g in st.galaxies["OE"].keys() if g not in st.galaxies
                        ]
                        if unexploredGalaxy:
                            dest = ["warp"]
            ...
            if st.orders["_rank"][ship["id"]] in st.galaxies:
                ...
                filteredTrades = list(
                    filter(
                        lambda x: x["type"] == "intra"
                        and isSameSystem(x["src"], st.orders["_rank"][ship["id"]])
                        and currentTrades[tradeStr(x)] < 2,
                        trades,
                    )
                )
                ...
                filteredTrades = list(
                    filter(
                        lambda x: x["type"] == "intra"
                        and isSameSystem(x["src"], ship["location"])
                        and ("fromStructureId" in x or "toStructureId" in x),
                        trades,
                    )
                )
                ...
                if not filteredTrades:
                    filteredTrades = list(
                        filter(
                            lambda x: currentTrades[tradeStr(x)]
                            < (2 if x["type"] == "intra" else 1),
                            trades,
                        )
                    )
            else:
            ...
            try:
                trade = random.choices(
                    filteredTrades,
                    list(map(lambda x: tradeWeight(x, ship), filteredTrades)),
                )
                if trade:
                    st.orders[ship["id"]] = createPathOrders(
                        ship["location"], trade[0]["src"]
                    )
                    st.orders[ship["id"]].append(trade[0])
                    st.orders["_live"][ship["id"]] = dict(
                        {"quantity": 0, "price": 0}, **trade[0]
                    )
    ...
    for ship in st.my.get("ships", []):
        if "location" in ship and st.orders.get(ship["id"], []) != []:
            ...
            if order.get("item"):
                quantity = min(
                    (
                        int(ship["maxCargo"] - max(maxFuelNeeded, activeFuel))
                        / st.goods[order["item"]]["volumePerUnit"]
                    ),
                    order.get("quantityAvailable", 5000),
                )
                if "fromStructureId" in order:
                    retJs = structureToShip(
                        ship,
                        order["fromStructureId"],
                        order["item"],
                        quantity,
                        ship["location"],
                    )
                ...
                st.orders["_live"][ship["id"]] = dict(
                    {
                        "quantity": quantity,
                        "price": quantity
                        * retJs.get("order", {}).get("pricePerUnit", 0),
                    },
                    **order
                )
    ...
    while loopCtr > 0:
        try:
            ...
            if "error" not in status:
                st.my = status
                if (
                    loopCtr % 60 == 0 and len(st.loans.get("active", [""])) > 0
                ):  # 20 * 30 = 1200sec -> 20dk+
                    updateLoans()
                sellAndDeposit()
                decideExpansion()
